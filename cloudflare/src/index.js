...
async function apiSignup(env, request, data) {
  ...
  // A child's email is OPTIONAL (most kids don't have one) — only validate if provided.
  if (kidEmail && !/^\S+@\S+\.\S+$/.test(kidEmail))
    return json({ error: "That child's email doesn't look right — you can leave it blank." }, 400);
